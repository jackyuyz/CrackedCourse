          ...
          <Card className="gap-0 py-0">
            <CardHeader className="border-border flex flex-col gap-3 border-b px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                <p className="text-muted-foreground text-[10px] font-bold tracking-[0.1em] uppercase">Writing notes for</p>
                <CardTitle className="mt-1 flex items-center gap-2 text-sm font-extrabold"><BookOpenText className="text-ocean size-4 shrink-0" /><span className="truncate">{selectedUnit.title}</span></CardTitle>
              </div>
              <div className="flex gap-5" role="tablist" aria-label="Note visibility">
                <button type="button" role="tab" aria-selected={visibility === "public"} onClick={() => selectVisibility("public")} className={cn("relative flex h-9 items-center gap-2 text-sm font-bold", visibility === "public" ? "text-navy" : "text-muted-foreground")}><Globe2 className="size-3.5" /> Public note{visibility === "public" ? <span className="bg-ocean absolute inset-x-0 bottom-0 h-0.5 rounded-full" /> : null}</button>
                <button type="button" role="tab" aria-selected={visibility === "private"} onClick={() => selectVisibility("private")} className={cn("relative flex h-9 items-center gap-2 text-sm font-bold", visibility === "private" ? "text-navy" : "text-muted-foreground")}><LockKeyhole className="size-3.5" /> Private note{visibility === "private" ? <span className="bg-ocean absolute inset-x-0 bottom-0 h-0.5 rounded-full" /> : null}</button>
              </div>
            </CardHeader>
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <Card className="gap-0 py-0 shadow-none">
              <CardHeader className="p-0">
                <button type="button" className="flex w-full items-center gap-3 px-5 py-3 text-left" onClick={() => setShowUnitDetails((current) => !current)} aria-expanded={showUnitDetails} aria-controls="unit-details-panel">
                  <span className="bg-sky/25 text-ocean grid size-8 place-items-center rounded-lg"><BookOpenText className="size-4" /></span>
                  <span className="min-w-0 flex-1"><span className="block text-sm font-extrabold">Unit details</span><span className="text-muted-foreground mt-0.5 block truncate text-xs">Rename, describe, or hide this unit</span></span>
                  <ChevronDown className={cn("text-muted-foreground size-4 transition-transform", showUnitDetails && "rotate-180")} />
                </button>
              </CardHeader>
              {showUnitDetails ? <CardContent id="unit-details-panel" className="border-border space-y-3 border-t p-4"><div className="space-y-1.5"><Label htmlFor="unit-title">Title</Label><Input id="unit-title" value={unitTitle} onChange={(event) => setUnitTitle(event.target.value)} disabled={demo || busy} maxLength={180} /></div><div className="space-y-1.5"><Label htmlFor="unit-description">Description <span className="text-muted-foreground">(optional)</span></Label><Textarea id="unit-description" value={unitDescription} onChange={(event) => setUnitDescription(event.target.value)} disabled={demo || busy} maxLength={1000} /></div><div className="flex flex-wrap gap-2"><Button type="button" size="sm" onClick={() => void saveUnit()} disabled={demo || busy || !unitTitle.trim()}>Save unit</Button><Button type="button" size="sm" variant="outline" onClick={() => void toggleUnitHidden(selectedUnit, true)} disabled={demo || busy}><ArchiveRestore className="size-3.5" /> Hide</Button></div></CardContent> : null}
            </Card>

            <Card className="gap-0 py-0 shadow-none">
              <CardHeader className="p-0">
                <button type="button" className="flex w-full items-center gap-3 px-5 py-3 text-left" onClick={() => setShowUnitMaterials((current) => !current)} aria-expanded={showUnitMaterials} aria-controls="unit-materials-panel">
                  <span className="bg-sky/25 text-ocean grid size-8 place-items-center rounded-lg"><FileText className="size-4" /></span>
                  <span className="min-w-0 flex-1"><span className="flex items-center gap-2 text-sm font-extrabold">Private materials <Badge variant="outline" className="bg-white text-[9px]">{selectedMaterials.length}</Badge></span><span className="text-muted-foreground mt-0.5 block truncate text-xs">PDFs, slide decks, and links for this unit</span></span>
                  <ChevronDown className={cn("text-muted-foreground size-4 transition-transform", showUnitMaterials && "rotate-180")} />
                </button>
              </CardHeader>
              {showUnitMaterials ? <CardContent id="unit-materials-panel" className="border-border space-y-3 border-t p-4">{selectedMaterials.map((material) => <MaterialRow key={material.id} material={material} courseId={initialData.courseId} demo={demo} busy={busy} onHide={hideMaterial} />)}{!selectedMaterials.length ? <p className="text-muted-foreground text-sm">No materials are attached to this unit yet.</p> : null}<Button type="button" variant="outline" size="sm" onClick={() => setShowMaterialForm((current) => !current)} disabled={demo || busy}><Plus className="size-3.5" /> Add material</Button>{showMaterialForm ? <form onSubmit={addMaterial} className="border-border space-y-3 rounded-xl border p-4"><div className="flex gap-2"><Button type="button" size="sm" variant={materialMode === "link" ? "default" : "outline"} onClick={() => setMaterialMode("link")}>Link</Button><Button type="button" size="sm" variant={materialMode === "file" ? "default" : "outline"} onClick={() => setMaterialMode("file")}>File</Button></div><div className="space-y-1.5"><Label htmlFor="material-title">Title</Label><Input id="material-title" value={materialTitle} onChange={(event) => setMaterialTitle(event.target.value)} maxLength={180} /></div>{materialMode === "link" ? <div className="space-y-1.5"><Label htmlFor="material-url">HTTPS link</Label><Input key="material-url" id="material-url" type="url" placeholder="https://…" value={materialUrl} onChange={(event) => setMaterialUrl(event.target.value)} /></div> : <div className="space-y-1.5"><Label htmlFor="material-file">PDF or slide deck</Label><Input key="material-file" ref={uploadInputRef} id="material-file" type="file" accept=".pdf,.ppt,.pptx,application/pdf,application/vnd.ms-powerpoint,application/vnd.openxmlformats-officedocument.presentationml.presentation" onChange={(event) => { const next = event.target.files?.[0] ?? null; setMaterialFile(next); if (next && !materialTitle) setMaterialTitle(next.name.replace(/\.[^.]+$/, "")); }} /><p className="text-muted-foreground text-xs">Private files only; maximum 20 MB.</p></div>}<Button type="submit" size="sm" disabled={busy || !materialTitle.trim() || (materialMode === "link" ? !materialUrl.trim() : !materialFile)}>{busy ? <LoaderCircle className="size-3.5 animate-spin" /> : materialMode === "file" ? <Upload className="size-3.5" /> : <LinkIcon className="size-3.5" />} Add private material</Button></form> : null}</CardContent> : null}
            </Card>
          </div>
