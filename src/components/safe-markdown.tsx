...
function inlineMarkdown(value: string): ReactNode[] {
  ...
  return parts.filter(Boolean).map((part, index) => {
    ...
    if (part.startsWith("==") && part.endsWith("==")) {
      return (
        <mark key={index} className="bg-gold/45 rounded-sm px-0.5 text-inherit">
          {part.slice(2, -2)}
        </mark>
      );
    }
    ...
    if (link) {
      ...
      return href ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-ocean hover:text-navy underline underline-offset-2"
        >
          {link[1]}
        </a>
      ) : (
  ...
  while (index < lines.length) {
    ...
    if (line.startsWith("```")) {
      ...
      blocks.push(
        <pre
          key={`code-${index}`}
          className="bg-navy overflow-x-auto rounded-lg p-3 text-xs text-white"
        >
          <code>{code.join("\n")}</code>
        </pre>,
      );
      ...
      blocks.push(
        <Heading
          ...
          className={cn(
            ...
            heading[1].length === 1 ? "mt-6 text-xl" : "mt-5 text-base",
          )}
        >
    ...
    if (line.startsWith("> ")) {
      blocks.push(
        <blockquote
          key={`quote-${index}`}
          className="border-ocean/50 text-muted-foreground border-l-2 pl-3 italic"
        >
    ...
    if (line.startsWith("|") && line.includes("|")) {
      ...
      const rows = tableLines
        .filter(
          (row, rowIndex) => rowIndex !== 1 || !/^\|?\s*:?-{3,}/.test(row),
        )
        .map((row) =>
          row
            .split("|")
      ...
      if (headings?.length) {
        blocks.push(
          <div key={`table-${index}`} className="overflow-x-auto">
            <table className="border-border min-w-full border-collapse text-left text-xs">
              <thead className="bg-muted">
                <tr>
                  {headings.map((heading, cellIndex) => (
                    <th
                      key={cellIndex}
                      className="border-border border px-2 py-1.5 font-bold"
                    >
